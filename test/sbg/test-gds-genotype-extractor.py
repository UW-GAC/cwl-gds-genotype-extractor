...
class Platform(unittest.TestCase):
    ...
    project_name = 'amstilp/gds-genotype-extractor-devel'
    task = None
    ...
    def setUpClass(cls):
        ...
        testdir = cls.session.files.query(names='testdir', project=cls.project)[0]
        print(testdir)
...
class PlatformLdIndex(Platform):
    ...
    execution = "platform"
    profile_name = 'bdc'
    log_filename = 'gds-genotype-extractor'
    input_filenames = {
        'gds_file': ['1KG_phase3_subset.gds'],
        'sample_include_file': ['sample_include.rds'],
        'variant_include_file': ['variant_include.rds']
    }
    other_input = {
    ...
    def test_outputs(self):
        ...
        if self.task.status == 'COMPLETED':
            ...
            out_expected_name = 'unittest_genotypes.rds'
            out_name = self.task.outputs['genotype_file'].name
